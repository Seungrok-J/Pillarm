...
>
> **진행 상태 (2026-08-28):** Feature 1·2 모두 구현되어 프로덕션에서 운영 중(2026-06-01 최초 배포). Claude Haiku 우선 호출·Sonnet 폴백 구조, 일일 5회 사용 제한(관리자는 무제한), 포(packet) 그룹화, 원터치 확인 UX까지 지속 개선됨.
> 2026-08-27 인식 결과 확인 화면을 Figma 디자인 기준으로 전면 개편 — 페이저 폭 겹침으로 텍스트가 화면 밖으로 나가던 문제 수정, 탭 글자 잘림 수정, 포를 테두리 카드로 분리해 "약국에서 받는 한 봉지" 단위로 읽히도록 재구성. 상세 UI/UX 요구사항은 `docs/scan-pack-ux-requirements.md` 참고.
...
- [x] 조제약 봉투 촬영 시 약 이름, 복용 횟수, 기간 인식 — 정량적 정확도(80%) 측정은 미실시, 정성적으로는 다중 약·표 형식 복약안내문까지 인식 가능(2026-08-18 max_tokens 상향으로 8개 이상 약 인식 시 응답 잘림 문제 해결)
- [x] 인식 결과는 반드시 사용자가 확인·수정 후 저장 (자동 저장 금지) — 원터치 요약 카드도 등록 버튼을 명시적으로 눌러야 저장됨
- [x] 약이 복수 인식된 경우 탭으로 전환하며 각각 확인 가능 — 탭 전환 + 좌우 스와이프 슬라이드까지 지원(2026-08-18)
- [x] API 오류 또는 인식 실패 시 수동 입력 화면으로 안내 ("직접 입력하기" 경로 제공)
- [x] 이미지는 서버 전송 전 최대 1024px로 리사이즈
- [ ] ~~로그인하지 않은 상태에서도 스캔 기능 사용 가능~~ — 정책 변경: 소셜 로그인 전용 전환(2026-06) 이후 스캔도 로그인 필수(`requireAuth`)로 통일
- [x] 스캔 결과로 만든 일정은 기존 일정과 동일하게 알림·기록·통계에 반영
- [x] 비용 제어: 하이쿠 모델 우선 사용, 실패 시에만 Sonnet 폴백(신뢰도 medium 표시)
