...
- 약 목록 표에 "투약량/횟수/일수"(또는 이와 유사한) 열이 있으면, timesPerDay는 그 "횟수"를 "일수"로 나눈 값을 사용하세요 (예: 횟수 6, 일수 3 → timesPerDay 2). 나눈 값이 정수가 아니면 반올림하세요. durationDays는 "일수" 값을 그대로 사용하세요.
- 이미지 안에 "①아침 ②점심 ③저녁 ④취침전"처럼 복용 시점을 체크하는 항목이 있고 그 중 일부가 체크(☑)되어 있다면, 체크된 항목만 mealSlots로 반환하세요(모든 약에 공통 적용). 체크 여부를 확인할 수 없으면 mealSlots는 null로 반환해 timesPerDay 기반 추론에 맡기세요.
- 이미지 안에 "식후 30분 / 식전 30분 / 공복 / 식후 즉시 / 식사 직전"처럼 복용 시점 관련 체크 항목이 있고 그 중 하나가 체크되어 있다면, 그 항목을 모든 약에 공통 적용해 withFoodMinutes(식후30분→30, 식전30분→-30, 식후즉시→0, 식사직전→-5, 공복→0이며 note에 "공복" 기재)를 계산하세요. 체크된 항목이 없으면 약별 복약안내 문구에서 판단하세요.
