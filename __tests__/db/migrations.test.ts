...
describe('runMigrations — 실행 흐름', () => {
  ...
  describe('신규 설치 (적용된 마이그레이션 없음)', () => {
    ...
    it('v1~v8 마이그레이션을 순서대로 실행한다', async () => {
      await runMigrations(asDb(db));

      // schema_migrations 생성(1) + v1~v8(8) = 9
      expect(db.execAsync).toHaveBeenCalledTimes(9);
    });

    it('v1~v8 버전을 schema_migrations 에 기록한다', async () => {
      await runMigrations(asDb(db));

      expect(db.runAsync).toHaveBeenCalledTimes(8);
      ...
      expect((db.runAsync.mock.calls[3][1] as unknown[])).toContain(4);
      expect((db.runAsync.mock.calls[4][1] as unknown[])).toContain(5);
      expect((db.runAsync.mock.calls[5][1] as unknown[])).toContain(6);
      expect((db.runAsync.mock.calls[6][1] as unknown[])).toContain(7);
      expect((db.runAsync.mock.calls[7][1] as unknown[])).toContain(8);
    });
  });

  // ── Phase 1 → Phase 2 업데이트 ──────────────────────────────────────────

  describe('v1만 적용된 상태 (v2~v8 업데이트)', () => {
    beforeEach(() => {
      db.getAllAsync.mockResolvedValue([{ version: 1 }]);
    });

    it('v2~v8 마이그레이션을 실행한다', async () => {
      await runMigrations(asDb(db));

      // schema_migrations(1) + v2~v8(7) = 8
      expect(db.execAsync).toHaveBeenCalledTimes(8);
    });

    it('v2~v8 버전을 schema_migrations 에 기록한다', async () => {
      await runMigrations(asDb(db));
  ...
  describe('v1~v8 모두 적용된 상태 (최신 버전)', () => {
    beforeEach(() => {
      db.getAllAsync.mockResolvedValue([
  ...
  describe('두 번 연속 호출 — 멱등성 보장', () => {
    it('첫 번째 호출 후 두 번째 호출은 아무것도 실행하지 않는다', async () => {
      ...
      expect(runAsyncCountAfterFirst).toBe(8);   // v1~v8 기록
      expect(runAsyncCountAfterSecond).toBe(8);  // 추가 기록 없음
    });
